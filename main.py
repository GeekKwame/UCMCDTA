...
def val_regression(test_dataset):
    ...
    return RMSE, Pearson, MSE, CI,rm2
...
if args.dataset == 'ki' or args.dataset == 'ic50':
    train_data, test_data, compound_len, protein_len = load_data(args.dataset, args.mode)

if args.dataset == 'kiba' or args.dataset == 'davis':
...
def main():
    ...
    for epoch in range(args.epoch):
        ...
        if args.mode == 'regression':
            ...
            if args.dataset == 'kiba' or args.dataset == 'davis':
                rmse_test,pearson_test,mse_test,ci_test,rm2_test= val_regression(test_data)
                print('Epoch:{:03d}'.format(epoch+1),
                      'train loss:{:.5f}'.format(lossTrain),
                      'rmse:{:5}'.format(str(rmse_test)),
                      'pearson:{:5}'.format(str(pearson_test)),
                      ...
                      'rm2:{:5}'.format(str(rm2_test)),

                      )
                rmse_val,pearson_val,mse_val, ci_val, rm2_val= test_regression(val_data)
                print(
                      'rmse:{:5}'.format(str(rmse_val)),
                      'pearson:{:5}'.format(str(pearson_val)),
                      'mse:{:5}'.format(str(mse_val)),
                      'ci:{:5}'.format(str(ci_val)),
                      'rm2:{:5}'.format(str(rm2_val)),
